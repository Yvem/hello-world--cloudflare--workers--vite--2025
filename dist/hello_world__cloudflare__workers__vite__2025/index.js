...
var pokemon$1 = {};
var uniqueRandom;
var hasRequiredUniqueRandom;
function requireUniqueRandom() {
  if (hasRequiredUniqueRandom) return uniqueRandom;
  hasRequiredUniqueRandom = 1;
...
function requirePokemon() {
  ...
  const randomNameGenerators = /* @__PURE__ */ new Map();
  function getLocalizedList(language = "en") {
    ...
    return commonjsRequire(`./data/${language.toLowerCase()}.json`);
  }
  pokemon$1.all = getLocalizedList;
  pokemon$1.random = (language = "en") => {
    if (randomNameGenerators.has(language)) {
      return randomNameGenerators.get(language)();
    }
    const list = getLocalizedList(language);
    const random = uniqueRandomArray2(list);
    randomNameGenerators.set(language, random);
    return random();
  };
  pokemon$1.getName = (id, language = "en") => {
    const list = getLocalizedList(language);
  ...
  pokemon$1.getId = (name, language = "en") => {
    const list = getLocalizedList(language);
    const index2 = list.indexOf(name);
    if (index2 === -1) {
      throw new Error(`Pokémon with name '${name}' does not exist for language code '${language}'. ${reportText}`);
    }
    return index2 + 1;
  };
  pokemon$1.languages = languages;
  return pokemon$1;
}
var pokemonExports = /* @__PURE__ */ requirePokemon();
const pokemon = /* @__PURE__ */ getDefaultExportFromCjs(pokemonExports);
const index = {
  fetch() {
